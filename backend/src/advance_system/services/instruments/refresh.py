...
class InstrumentMasterRefresher:
    ...
    Persistence stays outside this service so the accepted snapshot can later be
    written through the platform's chosen application/object-store adapter.
    """
    ...
    def _validate_candidate(
        ...
        rejected_ratio = snapshot.rejected_records / total_records if total_records else 1.0
    ...
    def refresh(self, url: str = UpstoxInstrumentMaster.COMPLETE_URL) -> InstrumentMasterSnapshot:
        ...
        candidate = self.loader.load_payload(payload, source_url=url)
    ...
    async def refresh_async(
        ...
        """Serialize refreshes so overlapping scheduler ticks cannot race."""
    ...
    async def run_periodically(
        ...
        *,
        ...
        """Refresh immediately, then continue at a fixed interval until stopped."""
        ...
        while not event.is_set():
            try:
                ...
                # Preserve the last known-good snapshot. The caller should expose
                # the failure through application observability/alerts.
                pass
            try:
